...
static rootblock_t *MakeRootBlock (DSTR diskname, BOOL requesters, globaldata *g);
...
BOOL FDSFormat (DSTR diskname, LONG disktype, SIPTR *error, BOOL requesters,
	globaldata *g)
{
  ...
  ULONG i;
	...
	if (!(rootblock = MakeRootBlock (diskname, requesters, g))) {
